    ...
    <ScrollView showsVerticalScrollIndicator={false} style={styles.container}>
      <Animatable.View animation="slideInUp" style={styles.containerImage}>
        <ScrollView
          ...
          {userMatched.photoUrl.map((image, index) => (
            <Image key={index} source={{uri: image}} style={{width, height}} />
          ))}
        </ScrollView>
        {userMatched.photoUrl.length > 1 && (
      ...
      <View style={styles.contentContainer}>
        <View style={styles.boxContainer}>
          <View style={styles.rowContainer}>
            <Text style={styles.textName}>{userMatched.firstName}</Text>
            <Text style={styles.textAge}>{userMatched.ages}</Text>
          </View>
        </View>
          ...
          <>
            <View style={styles.boxContainer}>
              <View>
                ...
                <Text style={styles.textContent}>{userMatched.aboutMe}</Text>
              </View>
            </View>
        ...
        <View style={styles.boxContainer}>
          <View>
            ...
            <View style={styles.containerTags}>
              {userMatched.tags.map(tag => (
                <View key={tag.id} style={styles.wrapTag}>
                  <Tag
                    tagName={tag.name}
                    active={profile.tags.some(item => item.id === tag.id)}
                    solid={false}
                  />
                </View>
              ))}
